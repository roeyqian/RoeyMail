  ...
  <div class="user-box">
    <div class="header-actions">
      ...
      <el-select v-model="params.status" placeholder="Select" class="status-select"
                 :style="`width: ${locale === 'en' ? 115 : 100 }px`">
    ...
    <el-dialog class="account-dialog" v-model="detailsShow" :title="t('userDetails')"  >
      <div class="details">
        ...
        <div>
          <span class="details-item-title">{{ $t('receiveCount') }}:</span>
          <span>{{ formatReceiveCount(userDetails) }}</span>
          <el-tag style="margin-left: 10px" v-if="userDetails.receiveAction.hasPerm">
            {{ formatReceiveType(userDetails) }}
          </el-tag>
          <el-button size="small" style="margin-left: 10px"
                     v-if="userDetails.receiveAction.hasPerm && userDetails.receiveAction.receiveCount"
                     @click="resetReceiveCount(userDetails)" type="primary">{{ $t('reset') }}
          </el-button>
        </div>
      </div>
    </el-dialog>
...
function formatReceiveType(user) {
  if (user.receiveAction.receiveType === 'day') return t('daily')
  if (user.receiveAction.receiveType === 'count') return t('total')
}

function formatReceiveCount(user) {

  if (!user.receiveAction.hasPerm) {
    return t('unauthorized')
  }

  if (!user.receiveAction.receiveCount) {
    return t('unlimited');
  }

  let count = user.receiveCount + '/' + user.receiveAction.receiveCount
...
function resetReceiveCount(user) {

  ElMessageBox.confirm(t('reReceiveConfirm', {msg: user.email}), {
    confirmButtonText: t('confirm'),
    cancelButtonText: t('cancel'),
    type: 'warning'
  }).then(() => {
    userRestReceiveCount(user.userId).then(() => {
      ElMessage({
        message: t('reSuccessMsg'),
        type: "success",
        plain: true
