    ...
    >
      <template #dropdown>
        <el-dropdown-menu>
          ...
          <el-dropdown-item v-if="props.type === 'all-email' && rightClickEmail.userEmail" @click="handleSearch('user', rightClickEmail.userEmail)">
            <template #default>
              <div class="right-dropdown-item">
...
function getEmailList(refresh = false) {
  ...
  props.getEmailList(emailId, queryParam.size).then(async data => {
    ...
    latestEmail.value = data.latestEmail || list.reduce((max, item) => item.emailId > (max?.emailId || 0) ? item : max, latestEmail.value);
