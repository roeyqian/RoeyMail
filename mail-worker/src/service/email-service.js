...
const emailService = {
	...
	async HandleOnSiteEmail(c, receiveEmail, sendEmailData, attList) {
		...
		for (const email of receiveEmail) {
			...
			if (accountRow) {
				...
				let { banEmail, availDomain, receiveType, receiveCount: roleReceiveCount } = roleRow;
				...
				if (email !== c.env.admin) {
					...
					} else if (roleReceiveCount) {
						const userRow = await userService.selectById(c, accountRow.userId);
						const currentReceiveCount = Number(userRow.receiveCount) || 0;
						if (currentReceiveCount >= roleReceiveCount) {
							emailValues.status = emailConst.status.BOUNCED;
							emailValues.message = `The recipient <${email}> has exceeded the receive limit.`;
						}
		...
		for (const emailData of receiveEmailList) {
			...
			// Increment receive count for successfully received emails
			if (emailData.userId > 0) {
				await userService.incrUserReceiveCount(c, 1, emailData.userId);
			}
	...
	async allList(c, params) {
		...
		if (userEmail) {
			conditions.push(sql`${email.userId} IN (SELECT user_id FROM user WHERE email COLLATE NOCASE LIKE ${'%' + userEmail + '%'})`);
		}
		...
		const queryCount = orm(c).select({ total: count() }).from(email).where(and(...countConditions));
	...
	async allEmailLatest(c, params) {
		...
		return await orm(c).select().from(email)
			.where(
				and(
					gt(email.emailId, emailId),
					eq(email.type, emailConst.type.RECEIVE),
