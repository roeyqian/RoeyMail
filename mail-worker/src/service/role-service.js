...
const roleService = {
	...
	selectByIdsHasReceivePermKey(c, types, permKey) {
		return orm(c).select({ roleId: role.roleId, receiveType: role.receiveType, receiveCount: role.receiveCount }).from(perm)
			.leftJoin(rolePerm, eq(perm.permId, rolePerm.permId))
			.leftJoin(role, eq(role.roleId, rolePerm.roleId))
			.where(and(eq(perm.permKey, permKey), inArray(role.roleId, types))).all();
	},
	...
	selectByIdsAndReceiveType(c, permKey, receiveType) {
		return orm(c).select({ roleId: role.roleId }).from(perm)
			.leftJoin(rolePerm, eq(perm.permId, rolePerm.permId))
			.leftJoin(role, eq(role.roleId, rolePerm.roleId))
			.where(and(eq(perm.permKey, permKey), eq(role.receiveType, receiveType))).all();
	},
