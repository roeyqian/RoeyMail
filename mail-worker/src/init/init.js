...
const dbInit = {
	...
	async v3_7DB(c) {
		try {
			await c.env.db.prepare(`UPDATE setting SET title = 'Roey Mail' WHERE title = 'Xi-Mail'`).run();
		} catch (e) {}
		try {
			await c.env.db.prepare(`UPDATE setting SET notice_title = 'Roey Mail' WHERE notice_title = 'Xi-Mail'`).run();
		} catch (e) {}
		try {
			await c.env.db.prepare(`UPDATE setting SET notice_content = REPLACE(notice_content, 'Xi-Mail', 'Roey Mail') WHERE notice_content LIKE '%Xi-Mail%'`).run();
		} catch (e) {}
	},

	async v3_8DB(c) {
		try {
			await c.env.db.prepare(`ALTER TABLE role ADD COLUMN receive_count INTEGER`).run();
		} catch (e) {}
		try {
			await c.env.db.prepare(`ALTER TABLE role ADD COLUMN receive_type TEXT NOT NULL DEFAULT 'count'`).run();
		} catch (e) {}
		try {
			await c.env.db.prepare(`ALTER TABLE user ADD COLUMN receive_count INTEGER NOT NULL DEFAULT 0`).run();
		} catch (e) {}
		try {
			await c.env.db.prepare(`INSERT INTO perm (name, perm_key, pid, type, sort) VALUES ('邮件接收', 'email:receive', 1, 2, 1.5)`).run();
		} catch (e) {}
	},
	...
	async v1_6DB(c) {
		...
		const noticeContent = 'Roey Mail 临时邮箱服务 🎉\n' +
			'<br>\n' +
			'加入 Telegram 频道获取最新动态：<a href="https://t.me/RoeyMail" target="_blank">@RoeyMail</a>\n' +
			'<br>\n' +
		...
		const ADD_COLUMN_SQL_LIST = [
			...
			`ALTER TABLE setting ADD COLUMN notice_title TEXT NOT NULL DEFAULT 'Roey Mail';`,
	...
	async intDB(c) {
		...
		try {
			await c.env.db.prepare(`
			  ...
			  SELECT 0, 0, 0, 0, 'Roey Mail', 0, 1, 1
